<div id="comments">
    ...
    <div id="respond" class="respond">
    	...
    	<form method="post" action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" id="comment_form">
    		...
    		<p><?php _e('登录身份：'); ?><?php printf(__('<a href="%1$s">%2$s</a>，'), get_option('siteurl') . '/wp-admin/profile.php', $user_identity); ?> <a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php _e('Log out of this account'); ?>"><?php _e('退出  &raquo;'); ?></p></a>
            <?php else: ?>
    		<p>
                <label for="author" class="required"><?php _e('昵称'); ?></label>
    			<input type="text" name="author" id="author" class="text" value="<?php echo $comment_author; ?>" required/>
    		</p>
    		<p>
                <label for="mail" class="required"><?php _e('邮箱'); ?></label>
    			<input type="email" name="email" id="mail" class="text" value="<?php echo $comment_author_email; ?>" required/>
    		</p>
    		<p>
                <label for="url"><?php _e('网站'); ?></label>
    			<input type="url" name="url" id="url" class="text" placeholder="<?php _e('http://example.com'); ?>" value="<?php echo $comment_author_url; ?>" required/>
    		</p>
            <?php endif; ?>
    		<p>
